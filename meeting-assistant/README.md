...
| **실시간 자막** | Deepgram Nova-3 STT 기반, 부분 결과 포함 라이브 표시 |
...
시스템 오디오 (SCStream)
    └─ AudioBufferProcessor (Actor, 48kHz→16kHz 리샘플링)
        └─ DeepgramEngine (WebSocket, Nova-3 STT)
...
(Deepgram 클라우드 STT 사용으로 on-device 모델 RAM 제로)
...
| 자막이 표시되지 않음 | Deepgram API 키 확인. 네트워크에서 `api.deepgram.com:443` 아웃바운드 허용 필요 |
