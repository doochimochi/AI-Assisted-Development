...
| **실시간 자막** | Google Cloud Speech-to-Text 기반 라이브 전사 |
...
| Google Speech API Key | `AIza...` | ✅ STT |
| Obsidian API URL | `http://192.168.X.X:27123` | Obsidian 저장 시 |
| Obsidian API Key | Obsidian 플러그인에서 복사 | Obsidian 저장 시 |

> Google Speech API 키 발급: console.cloud.google.com → **APIs & Services → Credentials → Create API Key** → 해당 프로젝트에서 **Cloud Speech-to-Text API** 활성화 필요
...
AudioRecorder (16kHz PCM, 250ms 청크)
    └─ GoogleSpeechClient (OkHttp REST, 침묵 감지 후 전송)
...
| 자막이 표시되지 않음 | Google Speech API 키 확인. `speech.googleapis.com:443` 허용 필요. Cloud Speech-to-Text API 활성화 여부 확인 |
